...
Puede ocurrir que se reciba la señal SIGTERM mientras se está bloqueado en el `.accept()`, como esta funcion es bloqueante, no se detectaría el cambio de estado hasta despues de comunicarse con un cliente. Para acatar este caso se agregó un loop con timeout en el método `__accept_new_connection(self)` que, en caso de ocurrir, chequea que el servidor no se esté apagando antes de volver a intentar conectarse. Si el servidor esta con `self.shutting_down=True` entonces sale del ciclo y de la función retornando `None`.
